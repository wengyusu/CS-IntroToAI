...
class Astar_Euclidean(object):
    def generate_EuclideanDistance(self, maze):
        '''
        set heuristic "h"
    ...
    def generate_path(self, maze):
        ...
        max_coorY = sizeY - 1
        max_coorX = sizeX - 1
        distanceE = self.generate_EuclideanDistance(maze)
        pq=[]
        path={}
        ...
        while len(pq)!=0:
            ...
            if node==(max_coorX,max_coorY):
                print("find path")
                break
            for (coor_y,coor_x) in self.neighbour(node,maze):
        ...
        return path
    def trace_back(self, maze):
        '''
        print the path when existed
        :param maze:
        :return:
        '''
        path=self.generate_path(maze)
        sizeY = len(maze)
        sizeX = len(maze[0])
        max_coorY = sizeY - 1
        max_coorX = sizeX - 1
